...
export const getImageUrlWithAiImg = async (
  oldImageUrl,
  aiImageUrl,
  preset = "collection_image",
  imageId = null
  ...
  // If no selected file and no oldImageUrl
  if (!aiImageUrl && !oldImageUrl) {
    return null;
    // If has oldImageUrl but no selected file
  } else if (oldImageUrl && !aiImageUrl) {
    return oldImageUrl;
    // If has/no imageUrl and selected file -> upload/update url
  } else {
    const match = oldImageUrl?.match(/\/([^/]+)\.png$/);
    ...
    if (match) {
      const publicId = match[1];
      try {
        const { data } = await updateImage(aiImageUrl, publicId, preset);
        const updatedUrl = data.secure_url;
        return updatedUrl;
      } catch (error) {
        console.log("error: ", error);
      ...
      const { data } = await uploadImage(aiImageUrl, imageId, preset);
      const updatedUrl = data.secure_url;
      return updatedUrl;
    }
  }
