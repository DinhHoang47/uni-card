...
export default function SettingModal({ isOpen, setIsOpen, collectionId }) {
  const speedLevel = {
    slow: 15,
    moderate: 10,
    fast: 5,
  ...
  const handleTest = () => {
    setIsOpen(false);
    router.push(
      `/collections/${collectionId}/test?mode=${mode}&show=${show}&time=${speed}`
    );
  };
...
// bottom-0 left-1/2 -translate-x-1/2 translate-y-10
